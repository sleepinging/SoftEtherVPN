...
void toHuman(UINT64 byte, wchar_t* buf, UINT size) {
    ...
    UINT i = 0;
    double d = (double)byte;
    while (d >= 1024.0 && i < sizeof(unit) / sizeof(unit[0]) - 1) {
        d /= 1024.0;
        i++;
    }
    swprintf(buf, size, L"%.3f %s", d, unit[i]);
}
...
void CmInitAccountListEx(HWND hWnd, bool easy)
{
    ...
    if (easy == false)
    {
        ...
        LvInsertColumn(hWnd, L_ACCOUNT, 5, _UU("CM_ACCOUNT_COLUMN_5"), 120);
    }
    else
    {
...
void CmRefreshAccountListEx2(HWND hWnd, bool easy, bool style_changed)
{
    ...
    if (CALL(hWnd, CcEnumAccount(cm->Client, &a)))
    {
        ...
        for (i = 0; i < a.NumItem; i++)
        {
            ...
            if (easy == false)
            {
                wchar_t usage_str[128];
                RPC_CLIENT_GET_CONNECTION_STATUS s;
                // Get the latest information
                Zero(&s, sizeof(s));
                UniStrCpy(s.AccountName, sizeof(s.AccountName), t->AccountName);
                if (CALL(hWnd, CcGetAccountStatus(cm->Client, &s)) == false || !s.Active)
                {
                    usage_str[0] = 0;
                }
                else {
                    // TotalRecvSize + TotalSendSize
                    UINT64 total = s.TotalRecvSize + s.TotalSendSize;
                    toHuman(total, usage_str, sizeof(usage_str));
                }

                LvInsertAdd(b, icon, (void*)t->StartupAccount, 6, t->AccountName,
